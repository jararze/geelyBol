                ...
                <div class="relative h-[500px] {{ $currentSlide['background_color'] ?? 'bg-gradient-to-br from-gray-100 via-gray-200 to-gray-300' }}">
                    ...
                    @if($promotionsData['navigation']['arrows_enabled'] ?? true)
                        <button wire:click="prevSlide"
                                class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-80 hover:bg-opacity-100 rounded-full p-3 transition-all text-blue-600 shadow-lg">
                        ...
                        <button wire:click="nextSlide"
                                class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-80 hover:bg-opacity-100 rounded-full p-3 transition-all text-blue-600 shadow-lg">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="{{ $promotionsData['navigation']['dots_wrapper_class'] ?? 'bg-gray-200 rounded-full px-4 py-2 flex space-x-2' }}">
                    @foreach($promotionsData['slides'] as $index => $slide)
                        <button wire:click="goToSlide({{ $index }})"
                                class="transition-all duration-300 {{ $this->currentSlide === $index ? ($promotionsData['navigation']['active_dot_style'] ?? 'w-8 h-3 bg-blue-600 rounded-full') : ($promotionsData['navigation']['dots_style'] ?? 'w-3 h-3 bg-gray-400 hover:bg-gray-500 rounded-full') }}">
                        </button>
                    @endforeach
                </div>
            </div>
